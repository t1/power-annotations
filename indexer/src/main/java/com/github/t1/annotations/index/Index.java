...
public class Index {
    public static Index load() {
        ...
        if (LOG.isLoggable(LEVEL)) {
            LOG.log(LEVEL, "------------------------------------------------------------");
            jandex.getKnownClasses().forEach(classInfo ->
                LOG.log(LEVEL, classInfo.name() + " :: " + classInfo.classAnnotations().stream()
    ...
    public Stream<AnnotationInstance> allAnnotationInstancesOfType(ClassInfo type) { return allAnnotationInstancesOfType(type.name()); }

    public Stream<AnnotationInstance> allAnnotationInstancesOfType(Class<?> type) { return allAnnotationInstancesOfType(toDotName(type)); }

    public Stream<AnnotationInstance> allAnnotationInstancesOfType(String typeName) { return allAnnotationInstancesOfType(toDotName(typeName)); }
    ...
    private MethodInfo methodInfo(org.jboss.jandex.MethodInfo methodInfo) {
        String[] typeNames = methodInfo.parameters().stream().map(Type::name).map(DotName::toString).collect(toArray(String.class));
