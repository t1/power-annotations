...
If a method or field is _not_ annotated with some annotation, the annotations from the class are considered as a fallback, both for `get(type)` and `all(type)`. When listing `all()` annotations on a method or field, class annotations are ignored.

*Feedback required*: should this only happen with an explicit annotation, e.g. `@FallbackToClass`.
